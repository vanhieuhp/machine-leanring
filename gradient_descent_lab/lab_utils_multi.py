...
plt.style.use('../sources/deeplearning.mplstyle')

def load_data_multi():
    data = np.loadtxt("sources/ex1data2.txt", delimiter=',')
    X = data[:,:2]
    y = data[:,2]
    return X, y
...
# compact extaction of hist sources
#x = hist["iter"]
...
def load_house_data():
    data = np.loadtxt("../sources/data/houses.txt", delimiter=',', skiprows=1)
    X = data[:,:4]
    y = data[:,4]
    return X, y
